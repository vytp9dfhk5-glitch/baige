...
从员工视角，登录后应直接获得由企业预置的姓名、手机号和默认职位，只需补充头像、个人简介、邮箱、微信号和公司地址等个人信息即可生成名片。员工可以申请修改职位，但在管理员审核通过前，名片必须继续展示原职位，并禁止产生新的对外分享。

从管理员视角，需要能够导入和维护员工目录、配置企业统一内容、管理角色、审批职位变更和名片续期、设置名片有效期，并能追踪申请状态。管理员不应依赖逐个手工创建账号，员工清单或 OA 数据应成为员工身份和默认职位的事实来源。
...
建设一套“个人信息由员工维护、企业内容由管理员统一、职位与有效期由管理员审核、访问行为可追踪”的智能电子名片系统。

员工通过员工清单中的手机号完成验证码登录。系统自动匹配员工目录中的姓名、手机号和默认职位，并在新增或编辑名片时预填。员工只能维护允许的个人字段；企业名称、企业介绍、宣传视频、企业资讯和其他统一企业内容由管理员配置，并自动合并到所有有效员工名片中。未填写的字段和空模块不展示。
...
管理员通过管理后台维护员工目录、角色、账号状态、企业统一内容、职位申请、续期申请和有效期。正式系统以员工目录数据库和服务端鉴权为安全边界；Excel 员工清单和 OA 接口是批量导入与同步来源。
...
2. As an employee, I want the system to verify that my phone number exists in the employee directory, so that only authorized company employees can use the product.
3. As an employee whose phone number is not in the directory, I want a clear rejection message, so that I know to contact an administrator instead of repeatedly trying to log in.
...
5. As an administrator, I want to assign administrator or employee roles to members, so that permissions match organizational responsibilities.
...
8. As an employee, I want my login phone number to be automatically filled and read-only, so that my business card identity stays consistent with my account.
...
15. As an administrator, I want to edit an employee's preset name, phone, default position, email, role, account status, and card validity, so that directory information can be corrected.
...
23. As an employee, I want to edit my email, WeChat ID, personal introduction, and company addresses, so that the card contains useful personal details.
...
92. As an employee, I want a separate visitor module, so that analytics are not mixed into the card editing page.
...
2. The core domain modules are Identity and Access, Employee Directory, Personal Card, Enterprise Content, Approval Workflow, Card Validity, Sharing, Add to Contacts, Visitor Analytics, Media, and Audit.
...
5. Unknown phone numbers are rejected at login. New employees enter the system only through administrator import, manual creation, or OA synchronization.
...
7. Account identity is keyed by immutable employee ID plus phone-number binding. Phone numbers must not be the only permanent database primary key because phone assignments can change.
...
9. Personal card records include avatar, display name, approved position reference, phone, optional email, optional WeChat ID, personal introduction, multiple company addresses, and publication status.
...
13. Position is not directly mutable on the published card. Employee edits create or update a pending position-change request.
...
20. Card validity is stored server-side and checked on owner-card load, visitor-card load, share-token creation, add-contact generation, and analytics access.
...
34. The owner application has employee navigation for Card, Visitors, and My. Administrators additionally see Management.
...
46. Labels and terminology use the latest approved wording: 个人信息、手机号、邮箱、微信号、公司地址、个人简介、公司风采、企业资讯、公司介绍、申请审批、名片有效期.
...
3. The negative onboarding seam is: login with a phone absent from the directory → verify access is rejected with an actionable message and no account is created.
...
5. Role tests cover primary administrator, promoted administrator, normal employee, disabled employee, and unknown phone.
6. Card-generation tests cover required fields, optional-field filtering, six default avatars, uploaded-avatar handling, multiple addresses, fuzzy address search, and enterprise-content merging.
...
24. Visual regression tests cover login, empty card, editor, generated card, pending approval, expired card, visitor list, visitor detail, administrator approval card, employee directory, My page, and every modal at representative phone sizes.
25. Production smoke tests cover real SMS/WeChat login, one imported employee, one administrator approval, one share, one public open, one add-contact action, and one visitor event before each release.
